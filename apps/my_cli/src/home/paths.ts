...
/** O PACOTE deste processo, por ÂNCORA e nunca por contar `../`.
 *
 ...
 *  em que alguém move o arquivo.
 *
 *  A ÂNCORA ERA `.git` E VIROU `package.json` em 20/08, quando o CLI desceu de
 *  `<repo>/src` pra `<repo>/apps/my_cli/src`. Num monorepo o `.git` é do
 *  MONOREPO — `code()` passaria a devolver `~/src/my`, e todo `join(code(), "src")`
 *  apontaria pro vazio SEM ERRO. `package.json` é a fronteira que este código
 *  realmente quer: o pacote de onde ele saiu.
 *
 *  Funciona igual quando o pacote É o repositório inteiro, que era o caso até hoje.
 *
...
export function code(from = import.meta.dir): Path {
	...
	for (let dir = from; ; dir = dirname(dir)) {
		...
		if (dirname(dir) === dir) throw new Error(`não achei o pacote (nenhum package.json de ${from} pra cima)`);
	}
}
...
export function resolve(): HomeSystem.Entities.Resolved {
	return {
		...
		why: {
			...
			code: process.env.MY_CODE ? "env:MY_CODE" : "anchor:package.json",
