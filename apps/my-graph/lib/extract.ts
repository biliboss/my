...
/** WHICH TREE TO DRAW. `MY_GRAPH_ROOT=~/src/my/packages/interfaces bun run dev` — a path,
...
/** A ÁRVORE DE CÓDIGO que os contratos deveriam cobrir — de onde saem os ÓRFÃOS.
 *
 *  ELA DEIXOU DE SER "a pasta acima dos contratos" em 20/08. Era isso enquanto
 *  contrato e código dividiam árvore (`src/interfaces/` dentro de `src/`); com os
...
// casa) pra `my/packages/interfaces` (pacote público), e este viewer saiu do
// checkout próprio pra `my/apps/my-graph` — o irmão ao lado. Agora o caminho é
// relativo DENTRO do monorepo, que é o único que não depende de onde alguém clonou.
