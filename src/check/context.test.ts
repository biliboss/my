...
cpSync(join(import.meta.dir, "../shared/argv.ts"), join(RAIZ, "src/shared/argv.ts"));
...
test("CONTEXT.md um acima do teto é ERRO e derruba o exit code", async () => {
  ...
  expect(out).toContain("02_areas/00_workflows/CONTEXT.md");
...
test("passando só o caminho staged, só ele é considerado", async () => {
  ...
  // enxergar o `02_areas/00_workflows/CONTEXT.md` que excede.
  const { out, code } = await exec(["CONTEXT.md"]);
  expect(out).not.toContain("02_areas/00_workflows/CONTEXT.md");
  expect(code).toBe(0);
});
