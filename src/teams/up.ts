...
//!
//! depends_on: src/teams/model.ts · src/teams/list.ts · src/herdr/workspaces/create.ts ·
//!             src/herdr/panes/grid.ts · src/agents/start.ts
...
import { type Lineup, type Team, agora, memberName, mint, storedOf, write } from "./model.ts";
import { find } from "./list.ts";

/** WHAT A MEMBER IS TOLD ON ITS FIRST BREATH. It names the queue and the two
 *  verbs that take from it — an agent that has to be told what to do next by a
 *  human is a pane, not a team member. */
const brief = (team: string, role: string, sub: TeamsSystem.ValueObjects.Subscription) =>
	[
		`You are the \`${role}\` of team \`${team}\`.`,
...
export async function up(lineup: Lineup): Promise<Team | Fail> {
	...
	for (const [i, role] of lineup.roles.entries()) {
		...
		const member = await start(memberName(name, role), { pane: panes.panes[i]!, prompt: brief(name, role, lineup.listens) }, launch);
...
export async function main(argv: string[]): Promise<number> {
	...
	const out = await up({
		name: opts.name,
		listens: { monitors, kinds: opts.kind },
		roles: cmd.args,
